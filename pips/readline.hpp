...
pips_readline(const char *prompt, std::vector<std::string> &history) {
#ifdef __unix__
  printf("%s", prompt);
  fflush(stdout);
  ...
  for (;;) {
    ...
    if (c == static_cast<char>(Key::NEWLINE) ||
        c == static_cast<char>(Key::CR)) {
      printf("\n");
      fflush(stdout);
      restore();
      return line + '\n';
    ...
    if (c == static_cast<char>(Key::BACKSPACE) ||
        c == static_cast<char>(Key::CTRL_H)) {
      if (cursor > 0) {
        line.erase(--cursor, 1);
        redraw();
      }
    ...
    if (c == static_cast<char>(Key::CTRL_A)) {
      cursor = 0;
      redraw();
      continue;
    } // move to start
    if (c == static_cast<char>(Key::CTRL_E)) {
      ...
      if (seq[0] == '[') {
        ...
        case 'B': // Down
          if (hist_pos < (int)history.size()) {
            ++hist_pos;
            line =
                (hist_pos == (int)history.size()) ? saved : history[hist_pos];
            cursor = (int)line.size();
            redraw();
          }
          break;
